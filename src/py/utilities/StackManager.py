...
class StackManager:
    ...
    def read_input_stacks(self, dir_input, filenames, suffix_mask):
        ...
        for i in range(0, self._N_stacks):
            self._stacks[i] = st.Stack.from_filename(
                os.path.join(dir_input, filenames[i] + ".nii.gz"),
                os.path.join(dir_input, filenames[i] + suffix_mask + ".nii.gz")
            )
    ...
    def read_input_stacks_from_slices(self, dir_input, prefixes_stacks, suffix_mask):
        ...
        for i in range(0, self._N_stacks):
            self._stacks[i] = st.Stack.from_slice_filenames(
                dir_input, prefixes_stacks[i], suffix_mask)
        ...
        for i in range(0, self._N_stacks):
            ...
            for j in range(0, N_slices):
                ...
                for k in range(0, N_cycles):
                    ...
                    rigid_motion_transforms[i][j][
                        k] = slice_rigid_motion_transforms[k]
    ...
    def get_total_number_of_slices(self):
        ...
        for i in range(0, self._N_stacks):
            ...
            N_begin_new_stack_array[i] = N_slices_total - \
                self._stacks[i].get_number_of_slices()
    ...
    def write_stacks(self, directory):
        ...
        # Write all slices
        for i in range(0, self._N_stacks):
            slices = self._stacks[i].write(directory=directory, filename=str(
                i), write_mask=True, write_slices=False)
