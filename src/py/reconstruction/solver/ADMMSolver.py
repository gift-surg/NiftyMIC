...
class ADMMSolver(Solver):
    ...
    # Get ouput directory to write TV results in case outputs of ADMM
    ...
    def run_reconstruction(self, verbose=0):
        ...
        if self._ADMM_iterations_output_dir is not None:
            print("ADMM iterations are written to " +
                  self._ADMM_iterations_output_dir)
        ...
        for iter in range(0, self._ADMM_iterations):
            ...
            if self._ADMM_iterations_output_dir is not None:
                ...
                HR_volume_itk = self._get_itk_image_from_array_vec(
    ...
    def _perform_ADMM_iteration(self, HR_nda, b, vx_nda, vy_nda, vz_nda, wx_nda, wy_nda, wz_nda, alpha, rho):
        ...
        HR_nda = self._perform_ADMM_step_1_TK1_recon_solution(
            HR_nda, b, vx_nda, vy_nda, vz_nda, wx_nda, wy_nda, wz_nda, rho)
        ...
        wx_nda, wy_nda, wz_nda = self._perform_ADMM_step_3_scaled_dual_variable(
            Dx_nda, Dy_nda, Dz_nda, vx_nda, vy_nda, vz_nda, wx_nda, wy_nda, wz_nda)
    ...
    def _perform_ADMM_step_1_TK1_recon_solution(self, HR_nda, b, vx_nda, vy_nda, vz_nda, wx_nda, wy_nda, wz_nda, rho):
        ...
        b[-3*self._N_voxels_HR_volume:] = np.sqrt(rho)*self._get_unscaled_b_lower(
            vx_nda, vy_nda, vz_nda, wx_nda, wy_nda, wz_nda)

        # Construct (sparse) linear operator A
        A_fw = lambda x: self._A_TK1(x, rho)
        A_bw = lambda x: self._A_adj_TK1(x, rho)

        # Run solver to compute least-square solution based on
        # TK1-regularization
    ...
    def _get_unscaled_b_lower(self, vx_nda, vy_nda, vz_nda, wx_nda, wy_nda, wz_nda):
        ...
        b[-3*self._N_voxels_HR_volume:-2 *
            self._N_voxels_HR_volume] = (vx_nda - wx_nda).flatten()
        b[-2*self._N_voxels_HR_volume:-
            self._N_voxels_HR_volume] = (vy_nda - wy_nda).flatten()
    ...
    def _vectorial_soft_threshold(self, ell, tx_nda, ty_nda, tz_nda):
        ...
        Sx[ind] = self._soft_threshold(
            ell, t_norm[ind])*tx_nda[ind]/t_norm[ind]
        Sy[ind] = self._soft_threshold(
            ell, t_norm[ind])*ty_nda[ind]/t_norm[ind]
        Sz[ind] = self._soft_threshold(
            ell, t_norm[ind])*tz_nda[ind]/t_norm[ind]
