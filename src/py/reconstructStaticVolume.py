...
def get_parsed_input_line(
    ...
    parser = argparse.ArgumentParser(description=
        ...
        version="0.1",
        usage="python reconstructVolume --dir_input=path-to-input-directory --dir_output=path-to-output-directory --iter_max=10 --alpha=0.02 --suffix_mask=_mask",
    ...
    parser.add_argument('--target_stack_index', type=int, help="Index of stack (image) in input directory (alphabetical order) which defines physical space for SRR. [default: %s]" %(target_stack_index), default=target_stack_index)
...
if __name__ == '__main__':
    ...
    ## Initial value
    HRvolume_init = stacks[0].get_isotropically_resampled_stack()
    HRvolume_init.set_filename("HR_volume_0")

    ## SRR step
    HRvolume = st.Stack.from_stack(HRvolume_init, filename="HR_volume")
    SRR = tk.TikhonovSolver(stacks, HRvolume, reg_type=args.regularization, minimizer="lsmr", iter_max=args.iter_max, alpha=args.alpha)
    ...
    HRvolume.set_filename(filename) 
    HRvolume.show()

    ##-------------------------------------------------------------------------
    HRvolume.write(directory=args.dir_output)
