...
def get_affine_registration_transform(fixed_sitk, moving_sitk, fixed_mask_sitk=None, moving_mask_sitk=None, print_info=False):
    ...
    registration_method.SetOptimizerAsRegularStepGradientDescent(
        learningRate=1, minStep=0.01, numberOfIterations=100)
...
class TestSimpleITKHelper(unittest.TestCase):
    ...
    def test_get_correct_itk_orientation_from_sitk_image(self):
        ...
        image_itk.SetDirection(
            sitkh.get_itk_direction_from_sitk_image(image_sitk))
        ...
        self.assertEqual(np.around(
            np.linalg.norm(np.array(image_sitk_from_itk.GetOrigin()) - image_sitk.GetOrigin()), decimals=self.accuracy), 0)

        # Check spacing
        self.assertEqual(np.around(
            np.linalg.norm(np.array(image_sitk_from_itk.GetSpacing()) - image_sitk.GetSpacing()), decimals=self.accuracy), 0)

        # Check direction matrix
        self.assertEqual(np.around(
            np.linalg.norm(np.array(image_sitk_from_itk.GetDirection()) - image_sitk.GetDirection()), decimals=self.accuracy), 0)
    ...
    def test_get_sitk_affine_transform_from_sitk_direction_and_origin(self):
        ...
        affine_transform_ref = sitkh.get_sitk_affine_transform_from_sitk_image(
            image_sitk)
        affine_transform = sitkh.get_sitk_affine_transform_from_sitk_direction_and_origin(
            direction, origin, image_sitk)

        # Check Fixed Parameters
        self.assertEqual(np.around(
            np.linalg.norm(np.array(affine_transform_ref.GetFixedParameters()) - affine_transform.GetFixedParameters()), decimals=self.accuracy), 0)

        # Check Parameters
        self.assertEqual(np.around(
            np.linalg.norm(np.array(affine_transform_ref.GetParameters()) - affine_transform.GetParameters()), decimals=self.accuracy), 0)
    ...
    def test_get_sitk_affine_transform_from_sitk_image(self):
        ...
        indices = np.array(
            np.where(sitk.GetArrayFromImage(image_sitk)[::-1] > 0))
        N_points = indices.shape[1]

        # Get sitk affine transform from image first (the way it is used in
        # Slice.py)
        affine_transform_sitk = sitkh.get_sitk_affine_transform_from_sitk_image(
            image_sitk)
        ...
        for i in range(0, N_points):
            ...
            # Check Alignment
            self.assertEqual(np.around(
                np.linalg.norm(np.array(affine_transform_sitk.TransformPoint(index)) - image_sitk.TransformIndexToPhysicalPoint(index)), decimals=self.accuracy), 0)
    ...
    def test_computation_point_physical_space_via_affine_transform(self):
        ...
        for i in range(0, N_points):
            ...
            # Check Alignment
            self.assertEqual(np.around(
                np.linalg.norm((A.dot(index) + t).flatten() - image_sitk.TransformIndexToPhysicalPoint(index.flatten())), decimals=self.accuracy), 0)
    ...
    def test_get_transformed_sitk_image(self):
        ...
        moving = st.Stack.from_filename(
            os.path.join(self.dir_test_data, moving_str + ".nii.gz"),
            os.path.join(self.dir_test_data, moving_str + "_mask.nii.gz"),
        )
        fixed = st.Stack.from_filename(
            os.path.join(self.dir_test_data, fixed_str + ".nii.gz"),
            os.path.join(self.dir_test_data, fixed_str + "_mask.nii.gz"),
        )
        # Register images
        ...
        affine_transform_sitk = get_affine_registration_transform(
            fixed.sitk, moving.sitk)
        rigid_transform_sitk = get_rigid_registration_transform(
            fixed.sitk, moving.sitk)
        ...
        moving_rigidly_warped_sitk = sitk.Resample(
            moving.sitk, fixed.sitk, rigid_transform_sitk, interpolator, default_pixel_value, moving.sitk.GetPixelIDValue())
        ...
        moving_rigidly_warped_sitkh_sitk = sitkh.get_transformed_sitk_image(
            moving.sitk, sitk.Euler3DTransform(rigid_transform_sitk.GetInverse()))
        moving_rigidly_warped_manual_sitk = sitk.Resample(moving_rigidly_warped_sitkh_sitk, fixed.sitk, sitk.Euler3DTransform(
        ), interpolator, default_pixel_value, moving.sitk.GetPixelIDValue())
        ...
        moving_affinely_warped_sitk = sitk.Resample(
            moving.sitk, fixed.sitk, affine_transform_sitk, interpolator, default_pixel_value, moving.sitk.GetPixelIDValue())

        # Resample after having transformed the moving image manually (inverse
        # transform)
        moving_affinely_warped_sitkh_sitk = sitkh.get_transformed_sitk_image(
            moving.sitk, sitk.AffineTransform(affine_transform_sitk.GetInverse()))
        moving_affinely_warped_manual_sitk = sitk.Resample(moving_affinely_warped_sitkh_sitk, fixed.sitk, sitk.Euler3DTransform(
        ), interpolator, default_pixel_value, moving.sitk.GetPixelIDValue())
