...
class RegistrationITK:
    ...
    def _run_registration_rigid_affine(self, id):
        ...
        if self._interpolator in ["OrientedGaussian"]:
            ...
            cov_HR_coord = psf.PSF().get_gaussian_PSF_covariance_matrix_reconstruction_coordinates( self._fixed, self._moving ).flatten()
    ...
    def get_stack_with_similarity_inplane_transformed_slices(self, stack_to_copy):
        ...
        stack_inplaneSimilar = st.Stack.from_sitk_image(stack_inplaneSimilar_sitk, filename=stack.get_filename(), image_sitk_mask=stack_inplaneSimilar_sitk_mask)
