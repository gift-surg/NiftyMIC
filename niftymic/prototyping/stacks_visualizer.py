...
class StacksVisualizer(object):
    ...
    #
    @classmethod
    def from_filenames(cls, filenames, labels=None):
    ...
    #
    def show_slice_select_directions(self,
                                     colors=None,
                                     title=None,
                                     fig_number=None):
        ...
        for i in range(0, self._N_stacks):
            ...
            slice_select_directions[i] = stack_sitk[:, :, -1:].GetOrigin() - \
                np.array(stack_sitk[:, :, 0:1].GetOrigin())

            # Normalize direction
            slice_select_directions[i] = slice_select_directions[
                i] / np.linalg.norm(slice_select_directions[i])
