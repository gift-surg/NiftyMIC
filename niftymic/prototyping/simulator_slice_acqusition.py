...
class SliceAcqusition:
    ...
    def __init__(self, HR_volume, output_spacing=(1, 1, 3.85), interpolator_type="OrientedGaussian", alpha_cut=3, motion_type="Random"):
        ...
        # Define default output spacing of slices and subsequently defined size
        # of simulated stack
        self._output_spacing = np.array(output_spacing)
        self._output_size = self._get_output_size_based_on_spacing(
        ...
        # Chosen cut-off distance for
        # itkOrientedGaussianInterpolateImageFunction
    ...
    def set_interpolator_type(self, interpolator_type):
        if interpolator_type not in ["NearestNeighbor", "Linear", "OrientedGaussian"]:
            raise ValueError(
                "Error: interpolator type can only be 'NearestNeighbor', 'Linear' or 'OrientedGaussian'")
    ...
    def set_motion_type(self, motion_type):
        if motion_type not in ["NoMotion", "Random"]:
            raise ValueError(
                "Error: motion type can only be 'NoMotion' or 'Random'")
        self._motion_type = motion_type
    ...
    def run_simulation_view_1(self):
        ...
        # Define coordinate system along which the slice acquisition shall be
        # simulated
        output_direction_sitk = np.array(
            [dir_in_plane_x, dir_in_plane_y, dir_slice_select]).transpose().flatten()
    ...
    def _run_stack_acquisition(self, output_direction_sitk, title="None"):
        ...
        for i in range(0, self._output_size[2]):
            ...
            #  is resampled
            rigid_motion_transform_itk = self._get_rigid_motion_transform[
                self._motion_type]()
            resampler.SetTransform(rigid_motion_transform_itk)

            # Set interpolator
            interpolator = self._get_interpolator[self._interpolator_type](
                output_direction_sitk, rigid_motion_transform_itk)
            ...
            nda_mask[i, :, :] = np.clip(
                np.round(sitk.GetArrayFromImage(slice_sitk_mask)), 0, 1)
            ...
            rigid_motion_transform_sitk = sitkh.get_sitk_from_itk_Euler3DTransform(
                rigid_motion_transform_itk)

            rigid_motion_transforms.append(rigid_motion_transform_sitk)

            # Store actually acquired position of slice within HR volume in
            # physical space
            affine_transform = self._get_ground_truth_affine_transform(
                output_direction_sitk, origin, slice_sitk, rigid_motion_transform_sitk)
        ...
        slice_range = self._get_rectangular_masked_region(
        ...
        self._affine_transforms.append(
            affine_transforms[slice_range[0]:slice_range[1]])
        self._rigid_motion_transforms.append(
            rigid_motion_transforms[slice_range[0]:slice_range[1]])
    ...
    def _get_interpolator_oriented_gaussian(self, output_direction_sitk, rigid_motion_transform_itk):
        ...
        oriented_gaussian_interpolator = itk.OrientedGaussianInterpolateImageFunction[
        ...
        motion_transform_sitk = sitkh.get_sitk_from_itk_Euler3DTransform(
            rigid_motion_transform_itk)

        # Get affine transform of slice given output transform (motion not
        # considered and only orientation important, hence dummy origin)
        affine_transform_slice_sitk = sitkh.get_sitk_affine_transform_from_sitk_direction_and_origin(
            output_direction_sitk, (0, 0, 0), self._output_spacing)

        # Get affine transform of simulated slice which will be sampled by
        # considering motion
        output_transformed_affine_sitk = sitkh.get_composite_sitk_affine_transform(
            motion_transform_sitk, affine_transform_slice_sitk)

        # Get image direction matrix of affine transform
        output_transformed_direction_sitk = sitkh.get_sitk_image_direction_from_sitk_affine_transform(
            output_transformed_affine_sitk, self._output_spacing)
    ...
    def _get_ground_truth_affine_transform(self, output_direction_sitk, output_origin_sitk, slice_sitk, motion_transform_sitk):
        ...
        affine_transform_slice = sitkh.get_sitk_affine_transform_from_sitk_direction_and_origin(
            output_direction_sitk, output_origin_sitk, slice_sitk)

        # Get ground truth affine transform of slice by taking into account
        # applied motion
        affine_transform = sitkh.get_composite_sitk_affine_transform(
            motion_transform_sitk, affine_transform_slice)
