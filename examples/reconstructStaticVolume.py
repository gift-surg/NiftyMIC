...
def get_parsed_input_line(
    ...
    parser.add_argument('--dir-input', type=str, help="Input directory with NIfTI files (.nii or .nii.gz).", required=True)
    parser.add_argument('--dir-output', type=str, help="Output directory. [default: %s]" %(dir_output), default=dir_output)
    parser.add_argument('--suffix-mask', type=str, help="Suffix used to associate a mask with an image. E.g. suffix_mask='_mask' means an existing image_i_mask.nii.gz represents the mask to image_i.nii.gz for all images image_i in the input directory. [default: %s]" %(suffix_mask), default=suffix_mask)
    parser.add_argument('--prefix-output', type=str, help="Prefix for SRR output file name. [default: %s]" %(prefix_output), default=prefix_output)
    parser.add_argument('--target-stack-index', type=int, help="Index of stack (image) in input directory (alphabetical order) which defines physical space for SRR. First index is 0. [default: %s]" %(target_stack_index), default=target_stack_index)
    ...
    parser.add_argument('--iter-max', type=int, help="Number of maximum iterations for the numerical solver. [default: %s]" %(iter_max), default=iter_max)
    parser.add_argument('--verbose', type=bool, help="Turn on/off verbose output. [default: %s]" %(verbose), default=verbose)
    parser.add_argument('--provide-comparison', type=bool, help="Turn on/off functionality to create files allowing for a visual comparison between original data and the obtained SRR. A folder 'comparison' will be created in the output directory containing the obtained SRR along with the linearly resampled original data. An additional script 'show_comparison.py' will be provided whose execution will open all images in ITK-Snap (http://www.itksnap.org/). [default: %s]" %(provide_comparison), default=provide_comparison)
